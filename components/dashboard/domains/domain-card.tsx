...
export function DomainCard({ domain, onClick }: TDomainCardProps) {
  const faviconUrl = `https://www.google.com/s2/favicons?domain=${domain.domain}&sz=64`;
    ...
    >
      <CardContent className="flex flex-col gap-3 p-5">
        ...
        <div className="flex items-center gap-3">
          <div className="relative size-9 shrink-0 rounded-lg bg-muted flex items-center justify-center overflow-hidden border border-border/40">
            <Image
              src={faviconUrl}
              alt={domain.domain}
              width={32}
            ...
            />
            <Globe className="absolute size-4 text-muted-foreground opacity-0 [img:not([style*='display: none'])~&]:hidden" />
          </div>
          <div className="min-w-0">
            <p className="font-semibold text-sm text-foreground truncate">{domain.domain}</p>
